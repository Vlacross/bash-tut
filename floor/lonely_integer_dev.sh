...
while IFS='\n' read line
  ...
  if [[ len -ne 0 ]]	#if the length isn't 0, we know the lengths were set but we haven't 
	...
	do
	  ...
	  echo "$line liners"	#grabbing items seperated by spaces
	  IFS=' '		
	...
	i=0
		echo ${#arr[@]}		#set outter loop starting counter
	while (( i<$arr_len ))		#outter loop range is length of array
	
#	for i in {0..${#arr[@]}}
 	  do
	    echo "running from $i index as ${arr[i]}"
...
#	    for j in {0..${#arr[@]}}
		...
		if [ "$occurance" -ge 2 ] && [ "${arr[j]}" == "${arr[i]}" ]	
		  ...
		  then					#remove from array to single
		    ...
		    echo "inner $j hit twice and unset"
#			echo ${arr[@]}
		fi
		...
		echo "OUTTER${arr[i]} $i hit twice and unset"
		echo ${arr[@]}			#when occurance isn't one, we assume 2 or more 
		echo "i is currently $i"	#matches found, so we remove the control element
		unset arr[i]			#we matched this loop around and increment our
	        i=$[ $i+1 ]			#outter loop to continue our search
	    fi
	  done

#	echo "final binal ${#arr[@]}"
	if [ -n $unique ] 
	  then
	    echo ${unique}
	fi
